...
`동영상 사이즈` 슬라이더로 타일 사이즈의 기준을 조정할 수 있습니다. 각 동영상은 잘라내지 않고 전체를 표시한 채로 행마다 높이를 조정하여 검은 틈이 나오지 않도록 깔아 넣습니다(세로 방향만 수% 정도의 비율 조정이 들어갈 수 있습니다). 같은 화면에서 가능한 한 별도의 동영상을 선택합니다. 동영상 수가 타일 수보다 적으면 중복됩니다.
